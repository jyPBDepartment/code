...
public interface PowerInfoDao extends JpaRepository<PowerInfoEntity, String> {
	...
	// 分页与模糊查询
	@Query(value = "select * from sas_power_info t where t.jur_name like ?1 and t.jur_code Like ?2 and t.sub_jur_code = ''",
	...
	//查询子菜单
	@Query(value = "select * from sas_power_info t where t.sub_jur_code=:id", nativeQuery = true)
	public List<PowerInfoEntity> findListById(@Param("id") String id);
	...
	public int findJurCode(@Param("subJurCode") String subJurCode);
