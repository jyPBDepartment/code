...
public interface AccountInfoDao extends JpaRepository<AccountInfoEntity, String>{
	...
	//查询分页
	@Query(value="select * from sas_account_info t where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.phone like ?2,1=1) and if(?3 !='',t.audit_status like ?3,1=1) order by t.create_date desc",
			countQuery="select count(*) from sas_account_info t where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.phone like ?2,1=1) and if(?3 !='',t.audit_status like ?3,1=1) order by t.create_date desc",nativeQuery = true)
