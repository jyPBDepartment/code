...
public interface PostCommentInfoDao extends JpaRepository<PostCommentInfoEntity, String>{
	...
	@Query(value="select * from sas_post_comment_info t where if(?1 !='',t.comment_content like ?1,1=1) order by t.comment_date desc ",
			countQuery="select count(*) from sas_post_comment_info t where if(?1 !='',t.comment_content like ?1,1=1) order by t.comment_date desc",nativeQuery = true)
