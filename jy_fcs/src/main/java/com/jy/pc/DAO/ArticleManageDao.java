...
public interface ArticleManageDao extends JpaRepository<ArticleManageEntity, String> {
	...
	// 移动端-条件查询文章管理信息列表（接口）
	@Query(value = "select * from sas_article_manage  t  where if(?1 !='',t.section_id like ?1,1=1) and status='0' order by t.create_date desc", countQuery = "select count(*) from sas_article_manage t  where if(?1 !='',t.section_id like ?1,1=1) and status='0' order by t.create_date desc", nativeQuery = true)
