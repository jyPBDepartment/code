...
public interface CaseInfoDao extends JpaRepository<CaseInfoEntity, String> {
	...
	//	查询看图识病列表
	@Query(value = "SELECT t.id,t.url,t.name,date_format(t.update_date,'%Y-%m-%d %H:%i:%s') as updateDate,date_format(t.create_date,'%Y-%m-%d %H:%i:%s') as createDate,praise_num as praiseNum,browse_num as browseNum,collection_num as collectionNum,if((select count(0) from sas_case_info_collection where case_id = t.id and collection_user_id in :#{#map['userId']}) > 0,1,0) as isUserCollection,if((select count(0) from sas_case_praise where case_id = t.id and praise_user_id in :#{#map['userId']}) > 0,1,0) as isUserPraise,if((select count(0) from sas_case_info_irrelevant where case_id = t.id and irrelevantn_user_id in :#{#map['userId']}) > 0,1,0) as isUserIrrelevant FROM sas_case_info t where if(?1 !='',t.name like ?1,1=1) and t.audit_status='1' and if(?2 !='',t.crops_type_code = ?2,1=1) and if(?3 !='',t.dip_type_code = ?3,1=1) and if(?4 ='3',t.is_selected = 1,1=1) order by case when ?4='1' then browse_num end desc,case when ?4='2' then create_date end desc,case when ?4='3' then selected_date end desc,case when ?4='4' then comment_num end desc,case when ?4='5' then praise_num end desc",
