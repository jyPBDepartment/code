...
public interface GrainTradingReplyDao extends JpaRepository<GrainTradingReplyEntity, String> {
	...
	@Query(value = "select t3.id,t3.reply_content as replyContent,t1.comment_content as commentContent,date_format(t3.reply_date,'%Y-%m-%d %H:%i:%s') as date,t3.reply_user_name as user,t3.status,t3.is_anonymous as isAnonymous,t2.name as title from sas_grain_trading_comment t1,sas_publication_info t2,sas_grain_trading_reply t3 where t1.aid=t2.id and t1.id = t3.comment_id and t3.status != -1 and t2.name like ?1 and t3.reply_user_name like ?2 and t3.reply_content like ?3 order by t3.reply_date desc", 
			countQuery = "select count(0) from sas_grain_trading_comment t1,sas_publication_info t2,sas_grain_trading_reply t3 where t1.aid=t2.id and t1.id = t3.comment_id and t3.status != -1 and t2.name like ?1 and t3.reply_user_name like ?2 and t3.reply_content like ?3 order by t3.reply_date desc", 
			nativeQuery = true)
