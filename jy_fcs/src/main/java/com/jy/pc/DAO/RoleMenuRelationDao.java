...
public interface RoleMenuRelationDao extends JpaRepository<RoleMenuRelationEntity, String> {
	...
	@Query(value = "select count(0) from sas_role_menu_relation t where t.role_id =:roleId", nativeQuery = true)
	public int hasRelationByRole(@Param("roleId") String roleId);
