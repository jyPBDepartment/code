...
public interface ClassificationDao extends JpaRepository<ClassificationEntity, String> {
	...
	// 分页与模糊查询
	@Query(value = "select * from sas_classification_info  t  where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.code like ?2,1=1) order by t.create_date desc", countQuery = "select count(*) from sas_classification_info t  where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.code like ?2,1=1) order by t.create_date desc", nativeQuery = true)
