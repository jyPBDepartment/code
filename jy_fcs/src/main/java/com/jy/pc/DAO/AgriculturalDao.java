...
//	搜索发布信息中农服信息(类型、类别)
	...
	@Query(value = "select t.id,t.address,t.url,t.name,t.contacts_user as contactsUser,t.create_date as createDate,t.update_date as updateDate from sas_publication_info t where t.id in (select arg_id from sas_grain_trading_collection where collection_user_id = ?1) order by t.create_date desc",
