...
//	搜索发布信息中农服信息(标题名称)
	@Query(value = "SELECT * FROM sas_publication_info t where t.transaction_type_code in :#{#map['type']} and t.transaction_category_code in :#{#map['category']} and if(?1 !='',t.name like ?1,1=1) and t.status in ('1','4') and if(?2 !='',t.transaction_type_code = ?2,1=1) and if(?3 !='',t.transaction_category_code = ?3,1=1) and if(?4 !='',t.identity_code = ?4,1=1) and if(?5 !='',t.address like ?5,1=1) order by t.create_date desc", countQuery = "select count(*) FROM sas_publication_info t where t.transaction_type_code in :#{#map['type']} and t.transaction_category_code in :#{#map['category']} and if(?1 !='',t.name like ?1,1=1) and t.status in ('1','4')  and if(?2 !='',t.transaction_type_code = ?2,1=1) and if(?3 !='',t.transaction_category_code = ?3,1=1) and if(?4 !='',t.identity_code = ?4,1=1) and if(?5 !='',t.address like ?5,1=1) order by t.create_date desc", nativeQuery = true)
	public Page<AgriculturalEntity> findAgriInfo(String name, String transactionTypeCode,
			String transactionCategoryCode,String identityCode,String address,Map<String, List<String>> map, Pageable pageable);
