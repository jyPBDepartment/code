...
public interface ModuleInfoDao extends JpaRepository<ModuleInfoEntity,String>{
	@Query(value="select * from sas_module_info t where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.status like ?2,1=1) order by t.create_date desc",
			countQuery="select count(*) from sas_module_info t where if(?1 !='',t.name like ?1,1=1) and if(?2 !='',t.status like ?2,1=1) order by t.create_date desc",nativeQuery = true)
