...
public interface GrainPricesDAO extends JpaRepository<GrainPricesEntity, String> {
	...
	@Query(value = "select * from sas_grain_prices_info t where if(?1 !='',t.price_defined_type = ?1,1=1) and if(?2 !='',t.area_id = ?2,1=1) order by t.create_date desc", countQuery = "select count(*) from sas_grain_prices_info t where if(?1 !='',t.price_defined_type like ?1,1=1) and if(?2 !='',t.area_id = ?2,1=1) order by t.create_date desc", nativeQuery = true)
