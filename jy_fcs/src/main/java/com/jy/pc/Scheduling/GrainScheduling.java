	...
	public void grainPrices() {
		...
		try {
			if (list.isEmpty()) {
				...
				if (newestInfo == null || StringUtils.isNullOrEmpty(newestInfo.getPrice())) {
					// 粮价数据未初始化或最新粮价数据异常
					logger.initCustomizedLog("粮价走势图", "自动更新", "历史数据异常，请联系系统管理员");
				} else {
					// 复制前一天的数据存入数据库中
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					GrainPricesEntity today = new GrainPricesEntity();
					...
					history.setOperateContent(today.toString());
					history.setOperateType("3");
					grainPricesDAO.save(today);
					grainPricesHistoryDAO.save(history);
					logger.initCustomizedLog("粮价走势图", "自动更新", "更新数据："+today);
				}
			} else {
				// 今日已更新粮价走势数据
