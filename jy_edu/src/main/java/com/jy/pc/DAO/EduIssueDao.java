...
public interface EduIssueDao extends JpaRepository<EduIssueInfoEntity, String> {
	...
	// 分页与模糊查询
	@Query(value = "select * from edu_issue_info t  where if(?1 !='',user_name like ?1,1=1) and if(?2 !='',t.issue_state = ?2,1=1) and if(?3 ='',1=1,t.issue_by like concat('%',?3,'%')) order by t.create_date desc", 
			countQuery = "select count(*) from edu_issue_info t  where if(?1 !='',user_name like ?1,1=1) and if(?2 !='',t.issue_state = ?2,1=1) and if(?3 ='',1=1,t.issue_by like concat('%',?3,'%')) order by t.create_date desc", nativeQuery = true)
	...
	@Query(value = "select * from edu_issue_info  t  where issue_state = 1 and if(?1 !='',user_name like ?1,1=1) and if(?2 ='',1=1,t.user_card like concat('%',?2,'%')) and if(?3 ='',1=1,t.certificate_id in (select id from edu_certificate_info where vocation_id = ?3 )) order by t.create_date desc", 
			countQuery = "select count(*) from edu_issue_info t  where issue_state = 1 and if(?1 !='',user_name like ?1,1=1) and if(?2 ='',1=1,t.user_card like concat('%',?2,'%')) and if(?3 ='',1=1,t.certificate_id in (select id from edu_certificate_info where vocation_id = ?3 )) order by t.create_date desc", nativeQuery = true)
	...
	@Query(value = "select * from edu_issue_info  where user_id =?1 and vocation_id=?2", nativeQuery = true)
	public EduIssueInfoEntity findInfo(String userId,String vocationId);
}
